...
function(file ,email, record, redirect, serverWidget, render,search,https) {	
  ...
  function get_url(){
    ...
    if (context.request.method === 'GET') {
      ...
      try {
        ...
        log.debug('id1', id1);
        log.debug('param', request.parameters);
        log.debug('req', context.request);
        ...
        var template = file.load('SuiteScripts/ItemReceipt.xml');
  ...
  //     	});
      ...
      log.debug('record id', request.parameters.id);
