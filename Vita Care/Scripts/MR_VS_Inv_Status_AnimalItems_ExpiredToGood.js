...
define(['N/record', 'N/search', 'N/runtime', 'N/log'], function (record, search, runtime, log) {
    function getInputData() {
        try {
            ...
            var searchObj = search.create({
                ...
                filters:
                    [
                        ["status", "anyof", "5"], // Expired
                        "AND",
                        ["item.isinactive", "is", "F"],
                        "AND",
                        ["formulatext: case when FLOOR({inventorynumber.expirationdate} - {today}) >= 15 then 'T' else 'F' END", "is", "T"],
                        "AND",
                        ["formulatext: {item.class}", "startswith", "Animal"]
                    ],
                columns:
                    [
                        search.createColumn({ name: "item", label: "Item" }),
                        search.createColumn({ name: "class", join: "item", label: "Profit Center" }),
    ...
    function reduce(context) {
        try {
            ...
            statusChangeRecord.setValue({ fieldId: 'previousstatus', value: 5 }); // Expired
            statusChangeRecord.setValue({ fieldId: 'revisedstatus', value: 1 }); // Good
