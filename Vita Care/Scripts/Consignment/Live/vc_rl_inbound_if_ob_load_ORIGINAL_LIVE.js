							...
							if (isConsignment) {
								...
								try {
									...
									try {
										...
										// 🔴 NEW LOGIC: READ FROM INVENTORY TRANSFER AND SET JSON ON SO
										...
										for (var i = 0; i < itLineCount; i++) {
											...
											var itItem = itRec.getSublistValue({
												sublistId: 'inventory',
												fieldId: 'item',
												line: i
											});
											...
											for (var j = 0; j < assignCount; j++) {
												...
												var quantity = invDetail.getSublistValue({
													sublistId: 'inventoryassignment',
													fieldId: 'quantity',
													line: j
												});

												var expirationDate = invDetail.getSublistValue({
													sublistId: 'inventoryassignment',
											...
											var jsonString = JSON.stringify(invArray);

											log.debug('Generated JSON', jsonString);

											// MATCH TO SO LINE
											for (var s = 0; s < soLineCount; s++) {

												var soItem = soRecToUpdate.getSublistValue({
													sublistId: 'item',
													fieldId: 'item',
													line: s
												});

												if (soItem.toString() == itItem.toString()) {

													log.debug('Matching SO Line Found', s);

													soRecToUpdate.setSublistValue({
														sublistId: 'item',
														fieldId: 'custcol_ns_invdet_data',
														line: s,
														value: jsonString
													});
...
*/
						...
						if (arrInventoryInbndDetails.length > 0) {
							arrInventoryInbndDetails.forEach(function (objData) {
								...
								if (objItemSearch && objItemSearch.length > 0) {
									...
									log.audit("Expiry date " + expiryDate);
									if (isExpired(expiryDate.replace('-', '').replace('-', '')))
										status = 5; //expired

									log.audit("Is expired? " + isExpired(expiryDate.replace('-', '').replace('-', '')))
	...
	*/
		...
		function isExpired(expDateStr) {
			...
			var day1 = (getTodayYYYYMMDD().slice(6, 8)); //parseInt
			log.debug("day1 no parse " + day1)
			...
			log.audit("Diff is " + (diffMs / (1000 * 60 * 60 * 24)));

			log.audit("Is expired? " + Math.floor(diffMs / (1000 * 60 * 60 * 24)) <= 90)
