 ...
 * @fileName MR || Semnox Integration Games Runner
 */
    ...
    function (sftp, file, record, search, log, runtime) {
        ...
        function map(context) {
            try {
                ...
                rows.forEach(function (r) {
                    ...
                    var site = r.Location;
                    var payment = r['Payment Method'] || 'No Payment';
                    var product = r['Game Name'];
                    var amount = r['Total Play(Amount)'];
                    var trxdate = r.Date;
        ...
        function getFileName() {
            try {
                ...
                var dateVal = script.getParameter({ name: 'custscript_vs_report_date2' });

                if (!dateVal) {
                    throw new Error('Missing report date');
                }

                var formattedDate = formatDate(dateVal);

                return 'Malahi_Games_Report_' + formattedDate + '.csv';
