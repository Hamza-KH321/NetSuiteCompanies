        ...
        const onRequest = (scriptContext) => {
            try {
                ...
                if (scriptContext.request.method === 'GET') {
                    ...
                    // Add a blank option as the default for Location
                    ...
                    var selectedDate = scriptContext.request.parameters.custpage_date; // Retrieve Date
                    var selectedLocation = scriptContext.request.parameters.custpage_location; // Retrieve Location

                    // Validate fields
                    if (!customer_id || !fileObj || !selectedDate || !selectedLocation) {
                        throw new Error('All fields are required. Please fill out all fields before submitting.');
                    }
                    
                    log.debug('Selected Date', selectedDate);
                    log.debug('Selected Location', selectedLocation);
                    log.debug('Selected Customer', customer_id);
                    ...
                    const csvFolderId = 2256; // Replace with the folder ID for CSV storage
                    ...
                    if (csvFileId && customer_id && selectedLocation) {
                        var mapReduceTask = task.create({ taskType: task.TaskType.MAP_REDUCE });
                        mapReduceTask.scriptId = 'customscript_vs_mr_consignment_new';
                        mapReduceTask.deploymentId = 'customdeploy1';
                        mapReduceTask.logFileID = newTextFileId;
                    
                        // Pass parameters to the Map/Reduce script
                        mapReduceTask.params = {
                            'custscript_vs_cons_file_id_new1': csvFileId,
                            'custscript_vs_cons_file_name1': csvFileName,
                            'custscript_vs_customer_id1': customer_id,
                            'custscript_vs_log_file_id1': newTextFileId,
                            'custscript_vs_location_id': selectedLocation,
                            'custscript_vs_date1': selectedDate
                        };
                        ...
                        log.error('Missing Required Data', 'Either the CSV file ID, customer ID, or location ID is missing.');
                    }
