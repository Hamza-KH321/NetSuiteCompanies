...
define(['N/record', 'N/file', 'N/search', 'N/runtime', 'N/format'], function(record, file, search, runtime, format) {
    ...
    // Global object to track lot quantities across lines
    var lotQuantities = {};
    ...
    function getLotDetails(itemId, requiredQuantity, customerLocation) {
        ...
        // Iterate through the lot search results
        inventorynumberSearchObj.run().each(function(result) {
            var lotNumberId = result.getValue({ name: 'internalid' });
            var inventoryNumber = result.getValue({ name: 'inventorynumber' });
            var availableQty = parseFloat(result.getValue({ name: 'quantityavailable' }));
    
            // Check if we've already deducted from this lot
            if (lotQuantities[lotNumberId] !== undefined) {
                availableQty = lotQuantities[lotNumberId];
            }
    
            // Log the available quantity before allocation
            log.debug('Before Allocation', `Item ID: ${itemId}, Lot: ${inventoryNumber}, Available Qty: ${availableQty}, Remaining Qty: ${remainingQty}`);
    
            if (remainingQty > 0 && availableQty > 0) { // Only allocate if there is remaining quantity and available quantity is positive
                // Allocate as much as possible from this lot
                var allocatedQty = Math.min(remainingQty, availableQty);
                if (allocatedQty > 0) { // Only push lots with a positive quantity
                    lotDetails.push({ lotNumber: lotNumberId, quantity: allocatedQty });
                    remainingQty -= allocatedQty;
    
                    // Log the allocated quantity and update global tracking
                    log.debug('Allocated Quantity', `Item ID: ${itemId}, Lot: ${inventoryNumber}, Allocated Qty: ${allocatedQty}`);
    
                    // Update the global lotQuantities object
                    lotQuantities[lotNumberId] = availableQty - allocatedQty;
    
                    // Log the updated available quantity after allocation
                    log.debug('After Allocation', `Item ID: ${itemId}, Lot: ${inventoryNumber}, Remaining Lot Qty: ${lotQuantities[lotNumberId]}`);
                }
            }
    
            // Continue fetching more lots if remaining quantity is still greater than 0
            return remainingQty > 0; // Keep fetching if more quantity is needed
        });
    ...
    function addInvoiceLineWithInventoryDetails(invoiceRecord, itemId, lotDetails, description, storeCode, logFileID) {
        try {
            ...
            appendToLogFile(logFileID, logMessage);
            log.error('ERROR in addInvoiceLineWithInventoryDetails function  for item ' + itemId, error);
