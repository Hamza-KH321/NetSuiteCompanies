...
define(['N/record', 'N/file', 'N/search', 'N/runtime', 'N/format'], function (record, file, search, runtime, format) {
        ...
        try {
            ...
            var inventorynumberSearchObj = search.create({
                ...
                columns: [
                    ...
                    search.createColumn({
                        name: "formuladate",
                        formula: "CASE    WHEN INSTR(LOWER({inventorynumber}), 'ob') > 0 AND {expirationdate} IS NULL THEN TO_DATE('01/01/1999', 'DD/MM/YYYY')   WHEN INSTR(LOWER({inventorynumber}), 'ob') > 0 THEN TO_DATE('01/01/1999', 'DD/MM/YYYY')   ELSE {expirationdate} END",
                        label: "Custom Exp Date",
            ...
            var inputData = consolidatedData.map(record => {
                ...
                if (remainingQty > 0) {
                    appendToLogFile(logFileID, `Insufficient lot quantities for item ${itemId} and store code ${record['Store Code']}`);
    ...
    function map(context) {
        try {
            ...
            var storeCode = inputData.record['Store Code'];
            var customerID = inputData.record['customerId'];

            // Perform the search for Store Code and Customer ID
            var customrecord_vs_add_new_store_consrepSearchObj = search.create({
                ...
                filters: [
                    ...
                    "AND",
                    ["custrecord_vs_store_customer", "anyof", customerID]
                ],
                columns: [
            ...
            customrecord_vs_add_new_store_consrepSearchObj.run().each(function (result) {
                storeName = result.getValue({ name: "name" });
                return false; // Exit the loop after the first match
            });

            // If no result is found, storeName will remain null
            inputData.record.storeName = storeName || 'Unknown'; // Set default value if no storeName found

            log.debug('Map Stage - Line Data', inputData);

            // Pass the updated data to the reduce stage
            context.write({ key: '1', value: JSON.stringify(inputData) });
        ...
        try {
            ...
            itemData.forEach(function (entry) {
                if (entry.record.lotDetails && entry.record.lotDetails.length > 0) {
                    addInvoiceLineWithInventoryDetails(context, invoiceRecord, entry.record['Item ID'], entry.record.lotDetails, entry.record['Store Code'], entry.record.logFileID, entry.record.storeName);
                } else {
                ...
                var logData = {
                    ...
                    storeName: entry.record.storeName,
    ...
    function addInvoiceLineWithInventoryDetails(context, invoiceRecord, itemId, lotDetails, storeCode, logFileID, storeName) {
        try {
            ...
            invoiceRecord.setCurrentSublistValue({ sublistId: 'item', fieldId: 'custcol_vs_storename', value: storeName });
