...
define(['N/record', 'N/search', 'N/runtime', 'N/log'], function (record, search, runtime, log) {
            ...
            var searchObj = search.create({
                ...
                filters:
                    [
                        ...
                        "AND",
                        ["status", "anyof", "2"], // Near Expiry
                        "AND",
                        ["available", "greaterthan", "0"],
                        "AND",
                        ["inventorynumber.expirationdate", "before", "today"]
                    ],
                columns:
                    [
    ...
    function reduce(context) {
        try {
            ...
            var statusChangeRecord = record.create({ type: record.Type.INVENTORY_STATUS_CHANGE, isDynamic: true });

            statusChangeRecord.setValue({ fieldId: 'location', value: locationId });
            statusChangeRecord.setValue({ fieldId: 'previousstatus', value: 2 }); // Previous Status Near Expiry
            statusChangeRecord.setValue({ fieldId: 'revisedstatus', value: 3 }); // Expired

            inventoryDetails.forEach(function (detail) {
                if (!detail.item || !detail.available || !detail.internalId) {

                    log.error({ title: 'Missing Required Fields', details: 'Item: ' + detail.item + ', Available: ' + detail.available + ', Internal ID: ' + detail.internalId });
                    return;
                }
                ...
                if (subrecord) {
                    ...
                    log.error({ title: 'Inventory Detail Subrecord Not Created', details: 'Item ID: ' + detail.item + ', Location: ' + locationId });
                }
