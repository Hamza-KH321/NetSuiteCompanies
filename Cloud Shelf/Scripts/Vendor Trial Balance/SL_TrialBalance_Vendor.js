 ...
 * @filename SL_TrialBalance_Vendor.js
 * @description Trial Balance Suitelet - Shows grouped debit/credit per account and entity
 */
    ...
    function (serverWidget, search, log, render, file, format, url, record, runtime) {
        ...
        function onRequest(context) {
            if (context.request.method === 'GET') {
                var form = serverWidget.createForm({ title: 'Trial Balance Report Vendor' });

                var suiteletUrl = url.resolveScript({
                    scriptId: 'customscript_vs_sl_vendor_trial_balance', // update with your script ID
                    deploymentId: 'customdeploy_vs_sl_vendor_trial_balance', // update with your deployment ID
            ...
            function renderPDF(context) {
                ...
                log.debug('Trial Balance Data', JSON.stringify(trialBalanceData));

                var renderer = render.create();
                var templateFile = file.load({ id: 'TrialBalance/TrialBalanceVendor.xml' }); // Update to your actual template file ID
            ...
            function renderCSV(context) {
                ...
                trialBalanceData.rows.forEach(function (group) {
                    csv += group.account + "\n";
                    csv += "Vendor,Initial Balance,Debit,Credit,Period Balance,Ending Balance\n";
            ...
            function fetchTrialBalanceData(fromDate, toDate, subsidiaryId) {
                ...
                var openingMap = {}; // key: account|vendor, value: balance
                var openingFilters = [
                    [
                ...
                var openingResults = openingSearch.run().getRange({ start: 0, end: 1000 });
                openingResults.forEach(function (res) {
                    var account = res.getText({ name: "account", summary: "GROUP" }) || "";
                    var vendor = res.getText({ name: "entity", summary: "GROUP" }) || "";
                    var key = account + '|' + vendor;
                ...
                resultData.rows = Object.keys(grouped).map(function (account) {
                    var vendors = grouped[account];

                    var totals = vendors.reduce(function (acc, v) {
